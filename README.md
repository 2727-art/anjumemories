...
Final Raid のボスに対するプレイヤー攻撃は、通常のダメージ計算を行わず固定の `RAID SIGNAL` として処理します。スキル種別、強化状況、ダメージ倍率によるボス HP への影響はなく、一定間隔のヒット入力で参加シグナルだけが加算されます。ギルド救援が増えると 1 回あたりの固定シグナル値が上がりますが、ボス HP の進行は救援ギルドのタイムライン演出で決まり、プレイヤー貢献率のパーセンテージ表示は行いません。
...
支援ランキングは終盤まで REDWOLF が上位に残り、残り 1:45 でエクスカリオンが救援参加し、残り 1:20 までに 1 位へ上がる演出です。残り 2:00 で TIMELIMIT パネルが赤色に変化し、残り 0:40 でボス HP が 0 になってボス表示と当たり判定が消え、ランキング更新も停止します。600 秒到達後に `ドールを解放する` ゲートで討伐報酬を保存して Opening Shop へ帰還します。
